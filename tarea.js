...
const sumarLikesDeUsuario = usuario => {
  ...
  usuario.posts = [{ likes: 1 }, { likes: 3 }, { likes: 4 }];
  return usuario.posts.reduce((total, post) => { return total + post.likes }, 0);
}
...
const sumarArray = (numeros, cb) => {
  ...
  let resultado = numeros.reduce((total, post) => { return total + post.likes }, 0);
  cb(resultado);
}
